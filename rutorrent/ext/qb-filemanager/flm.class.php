...
class FLM {
	...
	public function __construct() {
		...
		$this->workdir = FileUtil::addslash($this->workdir);
	...
	public function archive () {
		...
		$this->batch_exec(array("sh", "-c", escapeshellarg($this->fman_path.'/scripts/archive')." ".escapeshellarg(Utility::getExternal($bin))." ".
	...
	public function extract($archive, $target) {
		...
		$this->batch_exec(array("sh", "-c", escapeshellarg($this->fman_path.'/scripts/extract')." ".escapeshellarg(Utility::getExternal($bin))." ".
	...
	protected function make_temp() {
		...
		$tmp['dir'] = FileUtil::addslash($this->settings['tempdir']).'.rutorrent/.fman/'.$tmp['tok'].'/';
	...
	public function mediainfo ($file) {
		...
		$this->xmlrpc->addCommand( new rXMLRPCCommand('execute_capture', 
					array(Utility::getExternal("mediainfo"), $this->workdir.$file)));
	...
	public function readlog($session, $lpos) {
		...
		$log['file'] = FileUtil::addslash($this->settings['tempdir']).'.rutorrent/.fman/'.$session.'/log';
	...
	public function video_info($video_file) {
		...
		$this->xmlrpc->addCommand( new rXMLRPCCommand('execute_capture', 
					array(Utility::getExternal("ffprobe"), '-v', 0, '-show_format', '-show_streams', '-print_format', 'json' ,'-i', $video_file)));
		//$this->xmlrpc->success();
		...
		if($video['total_frames'] < 1) {
			...
			$this->xmlrpc->addCommand( new rXMLRPCCommand('execute_capture', 
					array(Utility::getExternal("ffprobe"), '-v', 0, '-show_streams', '-print_format', 'json', '-count_frames', '-i', $video_file)));
	...
	public function screenshots($file, $output) {
		...
		$uisettings = json_decode(file_get_contents(FileUtil::getSettingsPath().'/uisettings.json'), true);
		$settings = array();
		...
		$this->batch_exec(array("sh", "-c", escapeshellarg($this->fman_path.'/scripts/screens')." ".escapeshellarg(Utility::getExternal('ffmpeg'))." ".
	...
	public function send_file($file) {
		...
		if(($file === FALSE) || (($finfo = LFS::stat($fpath)) === FALSE)) {CachedEcho::send('log(theUILang.fErrMsg[6]+" - '.$fpath.'");',"text/html");}
	...
	public function sfv_check ($file) {
		...
		$this->batch_exec(array("sh", "-c", escapeshellarg(Utility::getPHP())." ".escapeshellarg($this->fman_path.'/scripts/sfvcheck.php')." ".
	...
	public function sfv_create ($file) {
		...
		$this->batch_exec(array("sh", "-c", escapeshellarg(Utility::getPHP())." ".escapeshellarg($this->fman_path.'/scripts/sfvcreate.php')." ".
							escapeshellarg($this->temp['dir'])." ".escapeshellarg($this->userdir.$file)));
	}
	...
	public function sdie($args = '') {
  	...
  	public function __destruct() {
		...
		if($this->shout) {CachedEcho::send(json_encode($this->output));}
   	}
